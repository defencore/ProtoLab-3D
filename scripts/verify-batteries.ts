...
const selections = parts.filter((part) =>
  ['lipo-battery', 'li-ion-cell', 'standard-battery'].includes(part.id),
);
if (selections.length !== 3) throw new Error('Expected three battery packages');
...
const output = process.argv[2] ?? '/tmp/protolab-battery-cases.json';

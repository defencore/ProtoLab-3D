...
const selections = parts.filter((part) => part.id === 'raspberry-pi');
if (selections.length !== 1) throw new Error('Expected Raspberry Pi package');
const cases = selections.flatMap((part) =>
  part.presets
    .filter((p) => p.parameters.detail === 'lightweight')
    .flatMap((preset) =>
...
const output = process.argv[2] ?? '/tmp/protolab-pi-lightweight-cases.json';

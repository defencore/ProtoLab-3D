"""Validate electromagnetic macros with a FreeCAD-enabled Python.

Prepare with: node --import tsx scripts/verify-electromagnetics.ts [cases.json]
Run with: python scripts/verify-electromagnetics.py [cases.json] [report.json]
"""
...
cases = json.loads(Path(sys.argv[1] if len(sys.argv) > 1 else '/tmp/protolab-electromagnetics-cases.json').read_text())
output = Path(sys.argv[2] if len(sys.argv) > 2 else '/tmp/protolab-electromagnetics-results.json')
...
with tempfile.TemporaryDirectory(prefix='protolab-electromagnetics-') as directory:
    for case in cases:
        record = {'name': case['name'], 'passed': False, 'codeSha256': hashlib.sha256(case['script'].encode()).hexdigest()}
        doc = App.newDocument('ElectromagneticsAudit')
        try:
            ...
            if len(children) > 1:
                ...
                doc.recompute()
            step = str(Path(directory) / 'electromagnetics.step')
            ...
            fcstd = str(Path(directory) / 'electromagnetics.FCStd')
            doc.saveAs(fcstd)

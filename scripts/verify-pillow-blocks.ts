...
const selections = parts.filter((part) => part.id === 'pillow-block-bearing');
const cases = selections.flatMap((part) =>
  [
    ...part.presets.filter((p) => p.parameters.housingStyle === 'kp'),
    { id: 'kp001-centred-insert', parameters: { ...part.defaults, insertOffset: 0 } },
    { id: 'kp001-longer-slots', parameters: { ...part.defaults, slotLength: 9 } },
  ].flatMap((preset) =>
    [{ id: 'default' }].map((state) => {
...
const output = process.argv[2] ?? '/tmp/protolab-pillow-blocks-cases.json';

...
const selections = parts.filter((part) => part.id === 'srs-connector');
if (selections.length !== 1) throw new Error('Expected SRS connector package');
...
const output = process.argv[2] ?? '/tmp/protolab-srs-cases.json';

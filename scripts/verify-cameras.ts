...
const selections = parts.filter((part) => ['camera'].includes(part.id));
if (selections.length !== 1) throw new Error('Expected camera package.');
...
const output = process.argv[2] ?? '/tmp/protolab-cameras-cases.json';

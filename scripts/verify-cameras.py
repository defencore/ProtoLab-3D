"""Validate camera macros with a FreeCAD-enabled Python.

Prepare with: node --import tsx scripts/verify-cameras.ts [cases.json]
Run with: python scripts/verify-cameras.py [cases.json] [report.json]
"""
...
cases = json.loads(Path(sys.argv[1] if len(sys.argv) > 1 else '/tmp/protolab-cameras-cases.json').read_text())
output = Path(sys.argv[2] if len(sys.argv) > 2 else '/tmp/protolab-cameras-results.json')
...
with tempfile.TemporaryDirectory(prefix='protolab-cameras-') as directory:
    for case in cases:
        record = {'name': case['name'], 'passed': False, 'codeSha256': hashlib.sha256(case['script'].encode()).hexdigest()}
        doc = App.newDocument('CameraAudit')
        try:
            ...
            if len(children) > 1:
                ...
                doc.recompute()
            step = str(Path(directory) / 'cameras.step')
            ...
            fcstd = str(Path(directory) / 'cameras.FCStd')
            doc.saveAs(fcstd)

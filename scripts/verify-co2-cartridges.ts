...
const selections = parts.filter((part) => ['co2-cartridge'].includes(part.id));
if (selections.length !== 1) throw new Error('Expected CO2 cartridge package');
...
const output = process.argv[2] ?? '/tmp/protolab-co2-cases.json';

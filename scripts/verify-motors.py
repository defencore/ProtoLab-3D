"""Validate motor macros with a FreeCAD-enabled Python.

Prepare with: node --import tsx scripts/verify-motors.ts [cases.json]
Run with: python scripts/verify-motors.py [cases.json] [report.json]
"""
...
cases = json.loads(Path(sys.argv[1] if len(sys.argv) > 1 else '/tmp/protolab-motors-cases.json').read_text())
output = Path(sys.argv[2] if len(sys.argv) > 2 else '/tmp/protolab-motors-results.json')
...
with tempfile.TemporaryDirectory(prefix='protolab-motors-') as directory:
    for case in cases:
        record = {'name': case['name'], 'passed': False, 'codeSha256': hashlib.sha256(case['script'].encode()).hexdigest()}
        doc = App.newDocument('MotorAudit')
        try:
            ...
            if len(children) > 1:
                ...
                doc.recompute()
            step = str(Path(directory) / 'motors.step')
            ...
            fcstd = str(Path(directory) / 'motors.FCStd')
            doc.saveAs(fcstd)

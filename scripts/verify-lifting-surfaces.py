"""Validate lifting-surface macros with a FreeCAD-enabled Python.

Prepare with: node --import tsx scripts/verify-lifting-surfaces.ts [cases.json]
Run with: python scripts/verify-lifting-surfaces.py [cases.json] [report.json]
"""
...
cases = json.loads(Path(sys.argv[1] if len(sys.argv) > 1 else '/tmp/protolab-lifting-surfaces-cases.json').read_text())
output = Path(sys.argv[2] if len(sys.argv) > 2 else '/tmp/protolab-lifting-surfaces-results.json')
...
with tempfile.TemporaryDirectory(prefix='protolab-lifting-surfaces-') as directory:
    for case in cases:
        record = {'name': case['name'], 'passed': False, 'codeSha256': hashlib.sha256(case['script'].encode()).hexdigest()}
        doc = App.newDocument('LiftingSurfaceAudit')
        try:
            ...
            if len(children) > 1:
                ...
                doc.recompute()
            step = str(Path(directory) / 'lifting-surfaces.step')
            ...
            fcstd = str(Path(directory) / 'lifting-surfaces.FCStd')
            doc.saveAs(fcstd)

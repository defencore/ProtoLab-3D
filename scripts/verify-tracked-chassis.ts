 ...
 * FULL=1 covers catalog variants, inspection states, suspension travel, idler adjustment and frame dimensions.
...
const selections = parts.filter((p) => p.id === 'tracked-chassis');
...
for (const part of selections) {
  ...
  if (process.env.FULL)
    for (const state of ['undercarriage', 'exploded', 'frame'])
      examples.push({ name: 'default', parameters: part.defaults, state });
  if (process.env.FULL)
    for (const patch of [
      { leftAngle: 8, rightAngle: -8 },
      { idlerAdjustment: 4 },
      { length: 350, roadCount: 6 },
      { detail: 'lightweight' },
      {
        length: 350,
        width: 250,
      ...
      { driveDiameter: 52, roadDiameter: 26, leftAngle: -8, rightAngle: 8 },
...
const output = process.argv[2] ?? '/tmp/protolab-tracked-chassis-cases.json';

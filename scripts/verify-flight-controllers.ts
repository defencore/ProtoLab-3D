...
const selections = parts.filter((part) => ['flight-controller'].includes(part.id));
if (selections.length !== 1) throw new Error('Expected flight-controller package.');
...
const output = process.argv[2] ?? '/tmp/protolab-flight-controllers-cases.json';

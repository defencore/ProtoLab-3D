"""Validate flight-controller macros with a FreeCAD-enabled Python.

Prepare with: node --import tsx scripts/verify-flight-controllers.ts [cases.json]
Run with: python scripts/verify-flight-controllers.py [cases.json] [report.json]
"""
...
cases = json.loads(Path(sys.argv[1] if len(sys.argv) > 1 else '/tmp/protolab-flight-controllers-cases.json').read_text())
output = Path(sys.argv[2] if len(sys.argv) > 2 else '/tmp/protolab-flight-controllers-results.json')
...
with tempfile.TemporaryDirectory(prefix='protolab-flight-controllers-') as directory:
    for case in cases:
        record = {'name': case['name'], 'passed': False, 'codeSha256': hashlib.sha256(case['script'].encode()).hexdigest()}
        doc = App.newDocument('ControllerAudit')
        try:
            ...
            if len(children) > 1:
                ...
                doc.recompute()
            step = str(Path(directory) / 'flight-controllers.step')
            ...
            fcstd = str(Path(directory) / 'flight-controllers.FCStd')
            doc.saveAs(fcstd)

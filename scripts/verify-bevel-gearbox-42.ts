 ...
 * FULL=1 covers catalog variants, inspection states, rotation and tooth sweep.
...
const selections = parts.filter((p) => p.id === 'bevel-gearbox-42');
...
for (const part of selections) {
  ...
  if (process.env.FULL)
    for (const state of ['internals', 'exploded', 'housing'])
...
const output = process.argv[2] ?? '/tmp/protolab-bevel-42-cases.json';

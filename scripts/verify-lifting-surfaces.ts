...
const selections = parts.filter((part) => part.id === 'lifting-surface');
if (selections.length !== 1) throw new Error('Expected the lifting-surface package.');
...
const output = process.argv[2] ?? '/tmp/protolab-lifting-surfaces-cases.json';

...
const selections = parts.filter((part) => part.id === 'srs-retainer');
if (selections.length !== 1) throw new Error('Expected SRS retainer package');
...
const output = process.argv[2] ?? '/tmp/protolab-srs-retainer-cases.json';

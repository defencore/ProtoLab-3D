"""Validate electronics macros with a FreeCAD-enabled Python.

Prepare with: node --import tsx scripts/verify-electronicss.ts [cases.json]
Run with: python scripts/verify-electronicss.py [cases.json] [report.json]
"""
...
cases = json.loads(Path(sys.argv[1] if len(sys.argv) > 1 else '/tmp/protolab-electronicss-cases.json').read_text())
output = Path(sys.argv[2] if len(sys.argv) > 2 else '/tmp/protolab-electronicss-results.json')
...
with tempfile.TemporaryDirectory(prefix='protolab-electronicss-') as directory:
    for case in cases:
        ...
        try:
            ...
            if len(children) > 1:
                ...
                doc.recompute()
            step = str(Path(directory) / 'electronicss.step')
            ...
            fcstd = str(Path(directory) / 'electronicss.FCStd')
            doc.saveAs(fcstd)

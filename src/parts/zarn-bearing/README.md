# Combined needle / axial roller bearing
...
Only the generic geometry SDK under `src/core` is shared. Run `npm run parts:check -- zarn-bearing` to check this package and `npm run parts:export -- zarn-bearing <output-folder>` to prepare a runnable handoff with the SDK and reference assets.

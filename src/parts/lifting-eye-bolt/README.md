...
Only the generic geometry SDK under `src/core` is shared. Run `npm run parts:check -- lifting-eye-bolt` to check this package and `npm run parts:export -- lifting-eye-bolt <output-folder>` to prepare a runnable handoff with the SDK and reference assets.

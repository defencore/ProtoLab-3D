...
Only the generic geometry SDK under `src/core` is shared. Run `npm run parts:check -- threaded-rod` to check this package and `npm run parts:export -- threaded-rod <output-folder>` to prepare a runnable handoff with the SDK and reference assets.

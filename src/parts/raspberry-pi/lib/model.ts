...
const models = data.filter((m) => m.id !== 'raspberry-pi-5') as unknown as Model[];
...
export function pieces(p: Parameters, state: string): Piece[] {
  ...
  const lightweight = p.detail === 'lightweight';
  const out: Piece[] = [],
    bores: Shape[] = [];
  ...
  for (const d of m.details) {
    ...
    if (d.kind === 'pads' || d.kind === 'header') {
      ...
      if (!lightweight) add(tongue, d.name + ' tongue', black, layer);
      const count = d.count ?? 4;
      for (let i = 0; !lightweight && i < count; i++) {
        const u =

...
export function geometry(state: string, indices?: number[]) {
  const group = new Group();
  const materials = new Map<string, MeshStandardMaterial>();
  native.components
    .filter((_, i) => !indices || indices.includes(i))
    .forEach((c) => {
...
export function python(state: string, indices?: number[]) {
  const components = native.components.filter((_, i) => !indices || indices.includes(i));
  const templateIds = new Set(components.map((c) => c.template));
  return [
    'import base64, zlib',
    ...
    `component_labels = ${JSON.stringify(components.map((c) => c.label))}`,
    `component_colors = ${JSON.stringify(components.map((c) => [1, 3, 5].map((i) => parseInt(c.color.slice(i, i + 2), 16) / 255)))}`,
  ].join('\n');
}

...
Only the generic geometry SDK under `src/core` is shared. Run `npm run parts:check -- spur-gear` to check this package and `npm run parts:export -- spur-gear <output-folder>` to prepare a runnable handoff with the SDK and reference assets.

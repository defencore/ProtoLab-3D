...
const models = data as unknown as Model[];
  ...
  for (const d of m.details) {
    ...
    if (d.kind === 'pads' || d.kind === 'header') {
      ...
      for (const [i, [x, y]] of coords.entries()) {
        ...
        if (d.kind === 'header') {
          const pinZ = d.down ? -d.h : m.board + 0.06;
          const pinH = d.down ? d.h + m.board + 0.5 : d.h;
          const baseZ = d.down ? -2.2 : m.board + 0.06;
          const headerLayer = d.down ? -2 : 2;
          add(
            box([0.64, 0.64, pinH], [x - 0.32, y - 0.32, pinZ]),
            `${d.name} pin ${i + 1}`,
            gold,
            headerLayer,
          );
          const base = box(
            ...
            [x - pitch / 2 + 0.03, y - pitch / 2 + 0.03, baseZ],
          );
          add(
            subtract(base, box([0.64, 0.64, 2.4], [x - 0.32, y - 0.32, baseZ - 0.1])),

...
Only the generic geometry SDK under `src/core` is shared. Run `npm run parts:check -- spherical-roller-bearing` to check this package and `npm run parts:export -- spherical-roller-bearing <output-folder>` to prepare a runnable handoff with the SDK and reference assets.

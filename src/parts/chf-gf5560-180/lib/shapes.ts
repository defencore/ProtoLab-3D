...
import { solidUnionMesh } from './mesh';
import { num } from '../../../core/geometry';
export type Vec = [number, number, number];
export type Shape =
  | { kind: 'worm'; root: number; tip: number; pitch: number; height: number }
...
function solid(s: Shape): Geom3 {
  switch (s.kind) {
    case 'worm': {
      const steps = Math.ceil((s.height / s.pitch) * 48);
      const rings: Vec[][] = Array.from({ length: steps + 1 }, (_, i) => {
        const z = (s.height * i) / steps,
          a = (2 * Math.PI * z) / s.pitch;
        return [
          [s.root - 0.08, -s.pitch * 0.3],
          [s.tip, -s.pitch * 0.12],
          [s.tip, s.pitch * 0.12],
          [s.root - 0.08, s.pitch * 0.3],
        ].map(([r, dz]) => [r * Math.cos(a), r * Math.sin(a), z + dz] as Vec);
      });
      const ridge = solid({ kind: 'loft', rings });
      return modeling.booleans.intersect(
        modeling.booleans.union(ridge, solid(cylinder(s.root, s.height, [0, 0, 0]))),
        solid(cylinder(s.tip + 0.2, s.height, [0, 0, 0])),
      );
    }
...
export function pythonShape(s: Shape): string {
  switch (s.kind) {
    case 'worm':
      return `_gf_worm(${s.root},${s.tip},${s.pitch},${s.height})`;
    case 'transform':

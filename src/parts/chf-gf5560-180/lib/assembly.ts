...
export function geometry(pieces: Piece[]): Group {
  if (!pieces.length) return new Group();
...
export function python(pieces: Piece[]): string {
  const lines = [
    ...
    '    return shape',
    'def _gf_worm(root, tip, pitch, height):',
    '    path = Part.Wire(Part.makeHelix(pitch,height,root).Edges)',
    '    coords = [(root-.08,-pitch*.30),(tip,-pitch*.12),(tip,pitch*.12),(root-.08,pitch*.30)]',
    '    points = [App.Vector(r,0,z) for r,z in coords]',
    '    profile = Part.Wire(Part.makePolygon(points+[points[0]]).Edges)',
    '    ridge = path.makePipeShell([profile],True,True)',
    // Separate the cylinder seam from the sweep profile; coincident seams can
    // make OCC silently drop the ridge during the fuse/clip operation.
    '    core = Part.makeCylinder(root,height)',
    '    core.rotate(App.Vector(0,0,0),App.Vector(0,0,1),90)',
    '    return core.fuse(ridge).common(Part.makeCylinder(tip+.2,height)).removeSplitter()',
    'components = []',
  ];

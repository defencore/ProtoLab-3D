...
Only the generic geometry SDK under `src/core` is shared. Run `npm run parts:check -- worm-drive` to check this package and `npm run parts:export -- worm-drive <output-folder>` to prepare a runnable handoff with the SDK and reference assets.

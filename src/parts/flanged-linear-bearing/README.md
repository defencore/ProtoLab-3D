...
Only the generic geometry SDK under `src/core` is shared. Run `npm run parts:check -- flanged-linear-bearing` to check this package and `npm run parts:export -- flanged-linear-bearing <output-folder>` to prepare a runnable handoff with the SDK and reference assets.

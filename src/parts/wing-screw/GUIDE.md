...
Only the generic geometry SDK under `src/core` is shared. Run `npm run parts:check -- wing-screw` to check this package and `npm run parts:export -- wing-screw <output-folder>` to prepare a runnable handoff with the SDK and reference assets.

...
export type Shape =
  | { kind: 'rotateY'; child: Shape; angle: number }
  ...
  | { kind: 'union' | 'subtract'; children: Shape[] };
...
function solid(s: Shape): Geom3 {
  switch (s.kind) {
    case 'rotateY':
      return modeling.transforms.rotateY((s.angle * Math.PI) / 180, solid(s.child));
    case 'box':
      ...
      if (
        ...
        holes.every(
          (h) =>
            h.kind === 'cylinder' &&
            h.axis === 'z' &&
            h.origin[2] <= base.origin &&
            h.origin[2] + h.height >= base.origin + base.depth,
        )
      ) {
        const section = modeling.booleans.subtract(
          modeling.primitives.polygon({ points: base.points }),
          ...holes.map((h) => {
            if (h.kind !== 'cylinder') throw new Error('Expected a cylindrical profile hole.');
        ...
        return modeling.transforms.translate(
          [0, 0, base.origin],
          modeling.extrusions.extrudeLinear({ height: base.depth }, section),
        );
      }
...
export function pythonShape(s: Shape): string {
  switch (s.kind) {
    case 'rotateY':
      return `_kp_rotate(${pythonShape(s.child)}, ${num(s.angle)})`;
    case 'box':

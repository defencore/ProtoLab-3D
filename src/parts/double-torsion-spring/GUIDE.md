...
Only the generic geometry SDK under `src/core` is shared. Run `npm run parts:check -- double-torsion-spring` to check this package and `npm run parts:export -- double-torsion-spring <output-folder>` to prepare a runnable handoff with the SDK and reference assets.

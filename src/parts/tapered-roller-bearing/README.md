...
Only the generic geometry SDK under `src/core` is shared. Run `npm run parts:check -- tapered-roller-bearing` to check this package and `npm run parts:export -- tapered-roller-bearing <output-folder>` to prepare a runnable handoff with the SDK and reference assets.

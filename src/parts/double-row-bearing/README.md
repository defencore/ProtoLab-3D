...
Only the generic geometry SDK under `src/core` is shared. Run `npm run parts:check -- double-row-bearing` to check this package and `npm run parts:export -- double-row-bearing <output-folder>` to prepare a runnable handoff with the SDK and reference assets.

...
Only the generic geometry SDK under `src/core` is shared. Run `npm run parts:check -- needle-bearing` to check this package and `npm run parts:export -- needle-bearing <output-folder>` to prepare a runnable handoff with the SDK and reference assets.

...
Only the generic geometry SDK under `src/core` is shared. Run `npm run parts:check -- radial-oil-seal` to check this package and `npm run parts:export -- radial-oil-seal <output-folder>` to prepare a runnable handoff with the SDK and reference assets.

...
Only the generic geometry SDK under `src/core` is shared. Run `npm run parts:check -- round-linear-guide` to check this package and `npm run parts:export -- round-linear-guide <output-folder>` to prepare a runnable handoff with the SDK and reference assets.

...
const part: PartDefinition = {
  ...
  subgroup: 'VACUUM HANDLING',
  icon: 'box',
  description: 'Flared suction cup and optional bellows-style skirt with an open suction passage.',
  ...
  validate(p, state) {
    ...
    if (n(p, 'diameter') <= n(p, 'neck') + 4 || n(p, 'neck') <= n(p, 'bore') + 2 * n(p, 'wall'))
      errors.push('Cup skirt and neck need positive wall thickness.');
    return errors;
  },

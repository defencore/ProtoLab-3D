...
Only the generic geometry SDK under `src/core` is shared. Run `npm run parts:check -- split-lock-washer` to check this package and `npm run parts:export -- split-lock-washer <output-folder>` to prepare a runnable handoff with the SDK and reference assets.

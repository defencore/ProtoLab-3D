...
import { helix, num } from '../../../core/geometry';
export type Vec = [number, number, number];
export type Shape =
  | { kind: 'spring'; radius: number; wire: number; height: number; turns: number }
  | { kind: 'transform'; child: Shape; translation: Vec; rotation: Vec }
  | { kind: 'loft'; rings: Vec[][] }
  | { kind: 'revolve'; profile: [number, number][]; sweep?: number }
...
export const union = (...children: Shape[]): Shape => ({
  kind: 'union',
  children: children.flatMap((child) => (child.kind === 'union' ? child.children : [child])),
});
...
function solid(s: Shape): Geom3 {
  switch (s.kind) {
    case 'spring':
      throw new Error('Springs are standalone components.');
    case 'transform':
      return modeling.transforms.translate(
        s.translation,
        modeling.transforms.rotate(
          s.rotation.map((a) => (a * Math.PI) / 180) as Vec,
          solid(s.child),
        ),
      );
    case 'loft': {
    ...
    case 'revolve':
      return modeling.extrusions.extrudeRotate(
        { segments: 96, angle: ((s.sweep ?? 360) * Math.PI) / 180 },
...
export function component(
  ...
  // Placement does not change a part's solid. Keep motion out of the CSG kernel
  // so repeated jaws and animated hinges reuse their local mesh.
  if (shape.kind === 'transform') {
    const child = component(shape.child, label, color);
    child.rotation.set(...(shape.rotation.map((v) => (v * Math.PI) / 180) as Vec), 'ZYX');
    child.position.set(...shape.translation);
    const group = new Group().add(child);
    group.name = label;
  ...
  if (shape.kind === 'spring') {
    const mesh = helix(shape.radius, shape.wire, shape.height, shape.turns, color);
    mesh.name = label;
    mesh.position.z = shape.height / 2;
...
export function pythonShape(s: Shape): string {
  switch (s.kind) {
    case 'spring':
      return `_cb_spring(${s.radius},${s.wire},${s.height},${s.turns})`;
    case 'transform':
    ...
    case 'loft': {
      ...
      const volume = faces.reduce(
        (v, f) =>
          v +
          new Vector3(...points[f[0]]).dot(
            new Vector3(...points[f[1]]).cross(new Vector3(...points[f[2]])),
          ),
        0,
      );
      const oriented = volume < 0 ? faces.map((f) => [...f].reverse()) : faces;
      return `Part.makeSolid(Part.makeShell([Part.Face(Part.makePolygon([App.Vector(*pts[i]) for i in face + [face[0]]])) for pts in [${JSON.stringify(points)}] for face in ${JSON.stringify(oriented)}]))`;
    }
    case 'revolve':
      return `Part.Face(Part.makePolygon([${[...s.profile, s.profile[0]].map(([r, z]) => vector([r, 0, z])).join(',')}])).revolve(App.Vector(0,0,0),App.Vector(0,0,1),${num(s.sweep ?? 360)})`;
    case 'box':

...
Only the generic geometry SDK under `src/core` is shared. Run `npm run parts:check -- extension-spring` to check this package and `npm run parts:export -- extension-spring <output-folder>` to prepare a runnable handoff with the SDK and reference assets.

...
Preview and FreeCAD use the same closed boundary. Keep the source mounting dimensions and the planar H/L root datums when editing tooth relief. Run `node --import tsx --test tests/reference-gears.test.ts` to check those surfaces, the six mounting references and 25 stock bore configurations, and the bevel-pair case in `tests/gear-bores.test.ts` for the seven shaft profiles. Prepare the five-case native stock-bore check with `node --import tsx scripts/verify-bevel-pair.ts --stock-bores`, then run `scripts/verify-bevel-pair.py` using a FreeCAD-enabled Python.
